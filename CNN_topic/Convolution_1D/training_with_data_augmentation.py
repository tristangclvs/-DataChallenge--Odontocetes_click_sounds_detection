...
# def load_and_preprocess_data(df, target_length): 
    ...
    for index, row in tqdm(df.iterrows(), desc="Loading and preprocessing data", unit="file"):
        ...
        augmented_audio = composer(audio, sr)

        data.append(audio)
        data.append(augmented_audio)
        labels.append(current_audio_label)
        labels.append(current_audio_label)
...
def load_and_preprocess_data(df, target_length): 
    ...
    for index, row in tqdm(df.iterrows(), desc="Loading and preprocessing data", unit="file"):
        ...
        data_list.extend([audio, augmented_audio])
        labels_list.extend([current_audio_label, current_audio_label])
...
if __name__ == "__main__":
    ...
    models_directory = Path.cwd() / "CNN_topic" / "models"
    model_name = "1d_cnn_l2_data_augmentation_pitch_shift.h5"
